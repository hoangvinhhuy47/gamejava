...
public final class PrettyPrinter2DArray {
    ...
    public PrettyPrinter2DArray(PrintStream out) {
        this(out, DEFAULT_AS_NULL);
    }

    public PrettyPrinter2DArray(PrintStream out, String asNull) {
        if ( out == null ) {
    ...
    public void print(int[][] table){
        String[][] logBoard = new String[table.length][table[0].length];
        for (int i=0; i<table.length; i++) {
            for (int j=0; j<table[0].length; j++){
                logBoard[i][j] = String.valueOf(table[i][j]);
            }
        }
        print(logBoard);
    }
